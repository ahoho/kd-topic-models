...
class Dbpedia(Data):
    def __init__(self, batch_size, data_path='', ctx=None, saveto='', **kwargs):
        self.saveto = saveto
        super(Dbpedia, self).__init__(batch_size, data_path, ctx)

    def load(self, path='./dbpedia_csv', features='BoW', match_avitm=False):
        if path[:2] == '~/':
            path = os.path.join(os.path.expanduser(path[:2]), path[2:])

        ### Specify the file locations
        train_path = path + '/dbpedia_csv_train.npz'
        val_path = path + '/dbpedia_csv_val.npz'
        test_path = path + '/dbpedia_csv_test.npz'
