...
class Agnews(Data):
    def __init__(self, batch_size, data_path='', ctx=None, saveto='', **kwargs):
        self.saveto = saveto
        super(Agnews, self).__init__(batch_size, data_path, ctx)

    def load(self, path='./ag_news_csv', features='BoW', match_avitm=False):
        if path[:2] == '~/':
            path = os.path.join(os.path.expanduser(path[:2]), path[2:])

        ### Specify the file locations
        train_path = path + '/ag_news_csv_train.npz'
        val_path = path + '/ag_news_csv_val.npz'
        test_path = path + '/ag_news_csv_test.npz'
