...
class YelpPolarity(Data):
    def __init__(self, batch_size, data_path='', ctx=None, saveto='', **kwargs):
        self.saveto = saveto
        super(YelpPolarity, self).__init__(batch_size, data_path, ctx)

    def load(self, path='./yelp_review_polarity_csv', features='BoW', match_avitm=False):
        if path[:2] == '~/':
